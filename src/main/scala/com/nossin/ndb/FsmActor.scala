...
object FsmActor {
  def props(persistenceId: String) = Props(new FsmActor(persistenceId))
}
class FsmActor(_persistenceId: String)(implicit val domainEventClassTag: ClassTag[DomainEvent])
  extends PersistentFSM[CountDownLatchState,Count,DomainEvent]
    with ActorLogging {
